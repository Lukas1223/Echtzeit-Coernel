...
const int gSemInitWert = 0;
const int gMaxPufferA = 5; //Produktpuffer der Erzeuger
const int gMaxPufferB = 3;
const int gMaxPufferC = 2;
...
sem_t gErzeugerPufferSemA;
sem_t gErzeugerPufferSemB;
sem_t gErzeugerPufferSemC;
int gMomPufferA = 0;
int gMomPufferB = 0;
int gMomPufferC = 0;

sem_t gVerbraucherSemaphoreA; // Produktzwischenpuffer als gVerbraucherSemaphoreA
sem_t gVerbraucherSemaphoreB;
sem_t gVerbraucherSemaphoreC;
...
struct semStruct {
	sem_t* semErzeugerPuffer;
	sem_t* semVerbraucherPuffer;
	int* puffer;
};
...
void * ErzeugerA(void *arg) {
	...
	struct semStruct semStructA;
	semStructA.semErzeugerPuffer = &gErzeugerPufferSemA;
	semStructA.semVerbraucherPuffer = &gVerbraucherSemaphoreA;
	...
	for (;;) { //diese Schleife läuft unendlich lange, bzw bis der Erzeugerthread beendet wird (wenn wir das Programm schließen)
		...
		pthread_create(&warteTaskA, NULL, WartenImErzeuger,
				(void *) &semStructA); //dieser Thread wartet darauf, das Produkt an den Verbraucher weiter zu geben. Er bekommt als Übergabe eine Referenz an den momentanen
...
void * ErzeugerB(void *arg) {	//Kommentare siehe A
	...
	sem_init(&gErzeugerPufferSemB, 0, gMaxPufferB + 1);

	struct semStruct semStructB;
	semStructB.semErzeugerPuffer = &gErzeugerPufferSemB;
	semStructB.semVerbraucherPuffer = &gVerbraucherSemaphoreB;
	semStructB.puffer = &gMomPufferB;

	for (;;) {
		sem_wait(&gErzeugerPufferSemB);
...
void * ErzeugerC(void *arg) {	//Kommentare siehe A
	...
	sem_init(&gErzeugerPufferSemC, 0, gMaxPufferC + 1);

	struct semStruct semStructC;
	semStructC.semErzeugerPuffer = &gErzeugerPufferSemC;
	semStructC.semVerbraucherPuffer = &gVerbraucherSemaphoreC;
	semStructC.puffer = &gMomPufferC;

	for (;;) {
		sem_wait(&gErzeugerPufferSemC);
		guarded_nanosleep(delay);
...
void * Verbraucher(void *arg) {	//hieran haben wir kaum etwas geändert, außer den guarded_nanosleep vor das sem_post zu schieben
	...
	while (1) {
		if (VerbraucherWartenTest()) { // Alle Plätze sind frei
			// Hole das Teil aus dem Puffer
			sem_post(&gVerbraucherSemaphoreA);
			sem_post(&gVerbraucherSemaphoreB);
			sem_post(&gVerbraucherSemaphoreC);
			// und verpacke
			guarded_nanosleep(delay);
...
// Initialisiere gVerbraucherSemaphoreAn fuer den Erzeugerpuffer
	sem_init(&gVerbraucherSemaphoreA, 0, gSemInitWert);
	sem_init(&gVerbraucherSemaphoreB, 0, gSemInitWert);
	sem_init(&gVerbraucherSemaphoreC, 0, gSemInitWert);

	sem_init(&gErzeugerPufferSemA, 0, gMaxPufferA + 1);
	sem_init(&gErzeugerPufferSemB, 0, gMaxPufferB + 1);
	sem_init(&gErzeugerPufferSemC, 0, gMaxPufferC + 1);
