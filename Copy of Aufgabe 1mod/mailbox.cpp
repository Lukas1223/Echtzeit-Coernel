...
void * Erzeuger(void *arg) {	//Kommentare siehe A
	struct Erzeuger Erz = *(struct Erzeuger *) arg;
	struct timespec = Erz.timespec;
	//pthread_t warteTaskC;
	mqd_t mbG = mq_open(gMbName, O_RDWR);

	sem_t erzeugerPufferSem;
	sem_init(&erzeugerPufferSem, 0, Erz.gMaxPuffer + 1);

	for (;;) {
		sem_wait(&erzeugerPufferSem);
		guarded_nanosleep(delay);
		std::cout << "part produced" + Erz.type + "..." << std::endl << std::flush;
		mq_send(mbG, (char *) new Event(Erz.type), sizeof(Event), 1);
		pthread_create(&Erz.erzeugerTask, NULL, WartenImErzeuger,
				(void *) &erzeugerPufferSem);
	}
	return NULL;
}
...
// Initialisiere Erzeuger

struct Erzeuger ErzeugerA;
	struct timespec tsErzeugerA;
	ErzeugerA.timespec = tsErzeugerA;
	//ErzeugerA.type = 0;
	tsErzeugerA.tv_sec = gErzeugerA_Dauer_sec;
	tsErzeugerA.tv_nsec = gErzeugerA_Dauer_nsec;
	ErzeugerA.erzeugerTask =erzeugerTaskA;
	ErzeugerA.gMaxPuffer = 0;

struct Erzeuger ErzeugerB;
	struct timespec tsErzeugerB;
	//ErzeugerB.type = {};
	ErzeugerB.timespec = tsErzeugerB;
	tsErzeugerB.tv_sec = gErzeugerB_Dauer_sec;
	tsErzeugerB.tv_nsec = gErzeugerB_Dauer_nsec;
	ErzeugerB.erzeugerTask =erzeugerTaskB;
	ErzeugerB.gMaxPuffer = 0;


struct Erzeuger ErzeugerC;
	struct timespec tsErzeugerC;
	ErzeugerC.timespec = tsErzeugerC;
	tsErzeugerC.tv_sec = gErzeugerC_Dauer_sec;
	tsErzeugerC.tv_nsec = gErzeugerC_Dauer_nsec;
	ErzeugerC.erzeugerTask =erzeugerTaskC;
	ErzeugerC.gMaxPuffer = 0;
...
// zuerst die Threads der Erzeuger
	pthread_create(&erzeugerTaskA, NULL, Erzeuger, (void *) &ErzeugerA);
	pthread_create(&ErzeugerB.erzeugerTask, NULL, Erzeuger, (void *) &ErzeugerB);
	pthread_create(&ErzeugerC.erzeugerTask, NULL, Erzeuger, (void *) &ErzeugerC);
