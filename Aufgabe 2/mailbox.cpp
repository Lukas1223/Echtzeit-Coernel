...
void* WartenImErzeuger(void* puffer) {
...
void * ErzeugerA(void *arg) {
	...
	for (;;) {//diese Schleife läuft unendlich lange, bzw bis der Erzeugerthread beendet wird (wenn wir das Programm schließen)
			  //die if-Abfrage überprüft, ob gerade ein Produkt im Puffer ist, sobald die maximale Puffergröße erreicht ist, darf nicht mehr weiter produziert werden
			  //an sich ist das eine selbstgeschriebene Semaphore, deshalb hab ich in Version 2 auch eine Semaphore dafür benutzt, die Wirkprinzipien sind gleich
		if (momPuffer <= gMaxPufferA) {
			...
			pthread_create(&warteTaskA, NULL, WartenImErzeuger,
					(void *) &momPuffer); //dieser Thread wartet darauf, das Produkt an den Verbraucher weiter zu geben. Er bekommt als Übergabe eine Referenz an den momentanen
										  // Puffer, damit er ihn verringern kann, sodass neue Teile produziert werden können
		}
	}
	return NULL;
}
